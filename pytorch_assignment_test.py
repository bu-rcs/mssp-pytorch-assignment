...
def test_problem1():
    ...
    assert(parameter_list[2].size() == torch.Size([5, 10]))
    assert(parameter_list[4].size() == torch.Size([1, 5]))

def test_problem2():
    model, accuracy = pta.problem2("dataset/2022-2023_NBA_Player_Stats_Regular_Allstar.csv", 4, 5, 42)
    expected_accuracy = 0.93284

    assert(isinstance(model, torch.nn.Module))
    assert(expected_accuracy == pytest.approx(accuracy))
...
def test_problem3():
    ...
    assert(accuracy == pytest.approx(0.9559386))
